...
def book_basket(user):
    ...
    for book in baskets:
        if book.user_id == user.user_id:
            ...
            if days > 14:
                print(f"\n{Fore.RED}Turite perlaikytą knygą {book.book_title}, todėl negalite imti naujų")
                return
            
    bf.list_books()

    try:
        book_title = input("\nĮveskite knygos pavadinimą kurią norite paimti: ")
        for book in books:
            if book.book_title == book_title:
                if book.book_unit <= 0:
                    print(f"\n{Fore.RED}Knygos šiuo metu nėra sandėlyje.")
                    return 
                # Patikrina ar jau turi tą knygą
                if any(book.book_title == book_title and book.user_id == user.user_id for book in baskets):
                    print(f"\n{Fore.RED}Jūs jau pasiskolinote šią knygą.")
                    return
                # Skolina
                book.book_unit -= 1
...
def return_book(user):
    ...
    if not user_baskets:
        print(f"\n{Fore.GREEN}Neturite pasiskolintų knygų.")
        return
    ...
    try:
        book_title = input("\nĮveskite knygos pavadinimą, kurią norite grąžinti: ")
...
def view_my_books(user):
    ...
    if not user_baskets:
        ...
        print(f"\n{Fore.CYAN}Jūsų pasiskolintos knygos:")
        for book in user_baskets:
